...
func executeproblemAndsolutions() {
	...
	fmt.Println(colorReset)
	endTime = time.Now()
	fmt.Println(colorGreen+"Time elapsed for FindNthNodeInOneScan where n = 7\t", endTime.Sub(startTime))
	fmt.Println(colorReset)
	...
	fmt.Println(colorReset)
	endTime = time.Now()
	fmt.Println(colorGreen+"Time elapsed for FindLoopInListBruteForceApproach (No loop) = ", endTime.Sub(startTime))
	fmt.Println(colorReset)

	/*
		startTime = time.Now()
		listWithLoop := singlylist.CreateListWithALoop()
		fmt.Println(listWithLoop.HeadNode.NextNode.NextNode.NextNode.NextNode.NextNode)
		...
		fmt.Println(colorReset)
		endTime = time.Now()
		fmt.Println(colorGreen+"Time elapsed for FindLoopInListBruteForceApproach (With loop) = ", endTime.Sub(startTime))
		fmt.Println(colorReset)
	*/
}
